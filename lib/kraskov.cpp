...
#define DBL_MAX 2000000
#define DBL_MIN -2000000
...
int neiE1(double *x, int i, double scal, int bs, double eps,  int *box, int *lis, int *mxi) {
  ...
  mp=lrint((xc+eps)*scal); if(mp>bs) mp=bs;
  mm=lrint((xc-eps)*scal); if(mm<0)  mm=0;
  mi=box[mp];
  while(mi>=0) {
...
void mi2(double **x, int N, int K,
  ...
  for (i=1;i<=K;i++) phi[i]=psi[i]-(dim-1)/(i*1.0);   // 
...
void mi2r(double **x, int N, int K,
  ...
  phi=(double*)calloc(K+1,sizeof(double));
  for (i=1;i<=K;i++) phi[i]=psi[i]-(dim-1)/(1.0*i);   // 
...
void mi_xnyn(double **x, int dimx, int dimy, int N, int K, 
  ...
  double scalx=0.0, scaly=0.0;
  int *nn;
  int d;

  int BOX,BOX1;
  int **box,**boxy=NULL,**boxx=NULL,*lis; // two dimensional boxes
  int *lisy=NULL,*lisx=NULL; // lists for two dimensions
  int *boxx1=NULL, *boxy1=NULL; // onedimensional boxes
  int *lisy1=NULL,*lisx1=NULL; // lists for one dimensions
  int *mxi=NULL, *myi=NULL; //accumulative lists of points in oned boxes
  int *ind=NULL,*indx=NULL,*indy=NULL; //indeces of original data (the data resorted during box creating)
  double epsilon;
  int inveps;
  ...
  phi=(double*)calloc(K+1,sizeof(double));
  for (i=1;i<=K;i++) phi[i]=psi[i]-1/(1.0*i);   // 
...
void mir_xnyn(double **x, int dimx, int dimy, int N, int K, 
  ...
  int BOX,BOX1;
  int **box=NULL,**boxy=NULL,**boxx=NULL,*lis=NULL; // two dimensional boxes
  int *lisy=NULL,*lisx=NULL; // lists for two dimensions
  int *boxx1=NULL, *boxy1=NULL; // onedimensional boxes
  int *lisy1=NULL,*lisx1=NULL; // lists for one dimensions
  int *mxi=NULL, *myi=NULL; //accumulative lists of points in oned boxes
...
void mi2h(double **x, int N, int K,
  ...
  for (i=1;i<=N;i++) phi[i]=psi[i]-(dim-1)/(1.0*i);   // 
...
void mi_d(double **x, int dim, int N, int K, float *mi_cr,
  ...
  for (ik=0;ik<K;ik++) {
    dxy1[ik]/=N;mi_cr[ik*2]=(1.0*dim-1)*psi[N]+psi[ik+1]-dxy1[ik];
    dxy2[ik]/=N;mi_cr[ik*2+1]=(1.0*dim-1)*psi[N]+phi[ik+1]-dxy2[ik];
  }
...
void kraskov_done(t_kraskov *kr) {
  int d=0;
  for (d=0;d<kr->dim;d++) sfree(kr->x[d]);
  sfree(kr->psi);
  sfree(kr->scal);
  sfree(kr->x);
};
...
void kraskov_prepare(t_traj *gmx_traj,t_kraskov* kr) {
  //normalization
  int natoms=gmx_traj->natoms;
  int dim=natoms*DIM;
  int N=gmx_traj->nframes;	
  int d,nat,gmxdim;
  int nfr;
  kr->N=N;
  kr->dim=dim;
  ...
  snew(kr->x,dim);
  x=kr->x;
  for (nat=0,d=0;nat<natoms;nat++) {
    for (gmxdim=0;gmxdim<DIM;gmxdim++,d++) {
      snew(x[d],N); 
      for (nfr=0;nfr<N;nfr++)
	x[d][nfr]=gmx_traj->x[nat][nfr][gmxdim];
    }
  }
  ...
  for (d=0;d<dim;d++) {
    me=s=0; for (nfr=0;nfr<N;nfr++) me+=x[d][nfr];
    me/=N;  for (nfr=0;nfr<N;nfr++) s+=(x[d][nfr]-me)*(x[d][nfr]-me);
    s/=(N-1);s=sqrt(s); 
    if (s==0) {;}
    for (nfr=0;nfr<N;nfr++) {
      x[d][nfr] = (x[d][nfr]-me)/s;
      if (x[d][nfr]<min[d]) min[d]=x[d][nfr]; 
      if (x[d][nfr]>max[d]) max[d]=x[d][nfr];
    }
    for (nfr=0;nfr<N;nfr++) x[d][nfr]=x[d][nfr]-min[d];
  }

  snew(kr->scal,dim);
  BOX1=N-5;
  for (d=0;d<dim;d++) kr->scal[d]=BOX1/(max[d]-min[d]); 
  sfree(max); sfree(min);
  
  snew(kr->psi,N+1);
  kr->psi[1]=-(double).57721566490153;
  for (nfr=1;nfr<N;nfr++) kr->psi[nfr+1]=kr->psi[nfr]+1/(1.0*nfr);
};
...
double kraskov_wrap(t_kraskov* kr, int a1, int a2, int param_k) {
  ...
  /*copy data because mirxnyn changes it */
  for (row=a1*DIM,d=0;row<a1*DIM+DIM;row++,d++) {
    snew(x[d],N); 
    x[d]=static_cast<double*>(memcpy(x[d],kr->x[row],N*sizeof(double)));
    scal[d]=kr->scal[row]; /* for scal it suffices to copy the pointers to the data */
  }
  for (row=a2*DIM;row<a2*DIM+DIM;row++,d++) {
    snew(x[d],N);
    x[d]=static_cast<double*>(memcpy(x[d],kr->x[row],N*sizeof(double)));
    scal[d]=kr->scal[row];
  } 
  ...
  /* 
     fprintf(stderr,"kr-data for (%d,%d)  ---------------------------------\n",a1,a2);
     for (i=0;i<N;i++) {
     for (d=0;d<dim;d++) 
     fprintf(stderr,"%10.5f ",x[d][i]);
     fprintf(stderr,"\n");
     };
     fprintf(stderr,"============================================\n");
  */
  /* computation of mutual information */
  mir_xnyn(x,dimx,dimy,N,K,kr->psi,kr->scal,&mir);
